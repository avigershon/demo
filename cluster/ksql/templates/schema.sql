...
CREATE table sport_events_per_min AS SELECT name, source_name, league_name, sport_name, minute, date, score, score_1, score_2, timestamp, id, count(*) AS events FROM sport_events window TUMBLING (size 60 second) GROUP BY name, source_name, league_name, sport_name, minut, date, score, score_1, score_2, timestamp, id;
