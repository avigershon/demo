...
helm install --name elasticsearch  --set image.es.tag=6.2.3 --set kibana.image.repository=docker.elastic.co/kibana/kibana-x-pack --set kibana.image.tag=6.2.3 --set kibana.ingress.enabled=false elasticsearch
#helm upgrade elasticsearch --set image.es.tag=6.2.3 --set kibana.image.repository=docker.elastic.co/kibana/kibana-x-pack --set kibana.image.tag=6.2.3 --set kibana.ingress.enabled=false elasticsearch
