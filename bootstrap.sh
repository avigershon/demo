...
helm install --name elasticsearch  --set image.es.repository=docker.elastic.co/elasticsearch/elasticsearch --set image.es.tag=6.2.3 --set common.env.ELASTIC_PASSWORD=changeme --set kibana.env.ELASTIC_PASSWORD=changeme --set kibana.image.repository=docker.elastic.co/kibana/kibana --set kibana.image.tag=6.2.3 --set kibana.ingress.enabled=false elasticsearch
#helm upgrade elasticsearch --set image.es.repository=docker.elastic.co/elasticsearch/elasticsearch --set image.es.tag=6.2.3 --set common.env.ELASTIC_PASSWORD=changeme --set kibana.env.ELASTIC_PASSWORD=changeme --set kibana.image.repository=docker.elastic.co/kibana/kibana --set kibana.image.tag=6.2.3 --set kibana.ingress.enabled=false elasticsearch
