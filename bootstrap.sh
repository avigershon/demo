...
helm upgrade kibana --set image.repository=docker.elastic.co/kibana/kibana --set image.tag=5.4.3 --set ingress.enabled=true,ingress.hosts[0]=kibana.local,ingress.annotations."kubernetes\.io/ingress".class=nginx --set env.ELASTICSEARCH_URL=http://elasticsearch-elasticsearch-client.default.svc.cluster.local:9200 stable/kibana
