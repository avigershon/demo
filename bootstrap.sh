...
helm upgrade kibana --set image.repository=docker.elastic.co/kibana/kibana --set image.tag=5.4.3 --set ingress.enabled=true --set ingress.hosts[0]=kibana.local --set ingress.annotations."kubernetes\.io/ingress\.class"=nginx --set ingress.annotations."kubernetes\.io/tls-acme"=true --set env.ELASTICSEARCH_URL=http://elasticsearch-elasticsearch-client.default.svc.cluster.local:9200 stable/kibana
